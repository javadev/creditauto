...
public class Calculator implements Serializable {
    ...
    public static BigDecimal calculateDownPayment(
            BigDecimal goodPrice, BigDecimal initPaymentValue, String initPaymentType) {

        BigDecimal result = null;
        if (goodPrice != null
                && initPaymentValue != null
                && TYPE_PER_AMOUNT.equals(initPaymentType)) {
            result = initPaymentValue.multiply(H100).divide(goodPrice, RoundingMode.CEILING);
        }
        if (goodPrice != null
                && initPaymentValue != null
                && TYPE_PER_CENT.equals(initPaymentType)) {
