...
public class EditBorder extends Border {
    ...
    private final class EditBorderBehavior extends AbstractBehavior {
        ...
        public void beforeRender(Component c) {
            ...
            if (model != null && model instanceof PropertyModel
            ...
            if (model != null && model instanceof CompoundPropertyModel) {
                ...
                if (clazz.isAnnotationPresent(Entity.class) && !clazz.getName().endsWith("User")) {
                    fc.add(getValidator(clazz,
                            fc.getId().replaceFirst("_id$", "").replaceAll("(.*)\\.(.*)", "$2")));
                } else if (clazz.getName().endsWith("In_instance") || clazz.getName().endsWith("User")) {
                    try {
                        String type = clazz.getDeclaredField(
                            fc.getId().replaceFirst("_id$", "").replaceAll("(.*?)\\.(.*)", "$1")).getType().getName();
                        if (type.endsWith("List")) {
                            String type2 = fc.getId().replaceFirst("_id$", "").replaceAll("(.*?)\\.(.*)", "$1");
                            type = clazz.getName().replaceAll("(.*\\.)(.*)", "$1")
                                + type2.substring(0, 1).toUpperCase() + type2.substring(1, type2.length() - 1);
    ...
    private final class JSR303Validator implements INullAcceptingValidator {
        ...
        public void validate(IValidatable v) {
            ...
            Set<ConstraintViolation<?>> constraintViolations = validator.validateValue(clazz, expression, value);
